...
self.addEventListener('fetch', e => {
  ...
  // 防旧缓存：网络优先，取新；断网时回退缓存（避免升级后一直用旧版 app.js）
  e.respondWith(
    fetch(e.request).then(res => { if (res && res.ok) { const clone = res.clone(); caches.open(CACHE).then(c => c.put(e.request, clone)) }; return res }).catch(() => caches.match(e.request))
  )
})
