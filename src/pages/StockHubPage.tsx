...
import { PackageSearch, ClipboardCheck, PackageX, TriangleAlert, Tags, Ruler, PackageOpen, TrendingDown, FileSpreadsheet } from 'lucide-react'
...
export default function StockHubPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <FeatureGrid
        items={[
          // 2026-09-14 去重：原来 10 张卡里有 **5 张**都指向 /inventory
          //（库存查询 / 临期预警 / 低库存预警 / 商品建档-编辑 / 条码管理）——点哪张都落到同一个页面。
          // 低库存与临期本来就是那个页面上的现成筛选，保留成直达链接（?filter=low / ?filter=expiring）；
          // 商品建档编辑、条码管理合并进「库存查询」，另加一条「批量导入 Excel」。
          { to: '/inventory', label: '库存查询', desc: '商品列表/批次/货位/价格', icon: PackageSearch, badge: lowCount },
          { to: '/inventory?filter=low', label: '低库存预警', desc: '缺货/低于缺货线商品', icon: TrendingDown, badge: lowCount },
          { to: '/inventory?filter=expiring', label: '临期预警', desc: 'N 天内到期批次提醒', icon: TriangleAlert, badge: expiringList.length },
