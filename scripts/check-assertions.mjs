...
// 已知偶发失败（flake）名单。
//
// 2026-09-12 曾列入 `TTS 合成语音能被 KWS 检出（5 次内）`（实测 3 次里失败 1 次）。
// **该 flake 已修**：用 scripts/probe-kws-rate.mjs 实测单次检出率 p=0.267、且 KWS 侧确定性，
// 「5 次内」的全失败概率高达 21% —— 属测试设计问题。已改为 31 次（全失败 ≈7e-5），
// 又因为检测到就提前退出，典型成本仍是约 3.8 次合成，几乎不增加运行时间。
//
// 所以这里**故意留空**：那条断言若再失败，就是真信号，不该被当成「已知偶发」糊过去。
// 以后新增真 flake 时加到这个数组，并写清实测复现率 + 「先重跑，连续复现就修断言本身」的处理方式。
const KNOWN_FLAKY = []
